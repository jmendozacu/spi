...
class AgeRange extends AbstractSource
{
    /**
    ...
    public function getAllOptions()
    {
        $options = $this->dataHelper
            ->getUnserializedStoreConfig(DataHelper::XML_PATH_ENABLED_AGE_RANGES);
        $value = 0;
        $this->_options = [['label' => 'Select Age Range...', 'value' => $value]];

        foreach ($options as $option) {
            $this->_options[] = ['label' => $option['age_from'] . '-' . $option['age_to'], 'value' => ++ $value];
        }
