<?php
return [
    'scopes' => [
        'websites' => [
            'admin' => [
                'website_id' => '0',
                'code' => 'admin',
                'name' => 'Admin',
                'sort_order' => '0',
                'default_group_id' => '0',
                'is_default' => '0'
            ],
            'base' => [
                'website_id' => '1',
                'code' => 'base',
                'name' => 'Main Website',
                'sort_order' => '0',
                'default_group_id' => '1',
                'is_default' => '1'
            ]
        ],
        'groups' => [
            [
                'group_id' => '0',
                'website_id' => '0',
                'code' => 'default',
                'name' => 'Default',
                'root_category_id' => '0',
                'default_store_id' => '0'
            ],
            [
                'group_id' => '1',
                'website_id' => '1',
                'code' => 'main_website_store',
                'name' => 'Infinity Scrubs',
                'root_category_id' => '2',
                'default_store_id' => '1'
            ]
        ],
        'stores' => [
            'admin' => [
                'store_id' => '0',
                'code' => 'admin',
                'website_id' => '0',
                'group_id' => '0',
                'name' => 'Admin',
                'sort_order' => '0',
                'is_active' => '1'
            ],
            'default' => [
                'store_id' => '1',
                'code' => 'default',
                'website_id' => '1',
                'group_id' => '1',
                'name' => 'Default Store View',
                'sort_order' => '0',
                'is_active' => '1'
            ]
        ]
    ],
    'system' => [
        'default' => [
            'general' => [
                'locale' => [
                    'code' => 'en_US'
                ]
            ],
            'dev' => [
                'static' => [
                    'sign' => '0'
                ],
                'front_end_development_workflow' => [
                    'type' => 'server_side_compilation'
                ],
                'template' => [
                    'minify_html' => '0'
                ],
                'js' => [
                    'merge_files' => '0',
                    'minify_files' => '0',
                    'minify_exclude' => '/tiny_mce/
Accept',
                    'session_storage_logging' => '0',
                    'translate_strategy' => 'dictionary'
                ],
                'css' => [
                    'minify_files' => '0',
                    'minify_exclude' => '
                    /tiny_mce/
                '
                ]
            ]
        ],
        'stores' => [

        ],
        'websites' => [
            'admin' => [
                'web' => [
                    'routers' => [
                        'frontend' => [
                            'disabled' => 'true'
                        ]
                    ],
                    'default' => [
                        'no_route' => 'admin/noroute/index'
                    ]
                ]
            ],
            'base' => [
                'carriers' => [
                    'tablerate' => [
                        'import' => '1539895754,,,,4,0'
                    ],
                    'temando' => [
                        'collectionpoints_countries' => NULL
                    ],
                    'dhl' => [
                        'doc_methods' => NULL
                    ]
                ]
            ]
        ]
    ],
    'modules' => [
        'Magento_Store' => 1,
        'Magento_Directory' => 1,
        'Magento_Eav' => 1,
        'Magento_Theme' => 1,
        'BinaryAnvil_Base' => 1,
        'BinaryAnvil_BaseAdmin' => 1,
        'Magento_Customer' => 1,
        'Magento_Backend' => 1,
        'BinaryAnvil_Federation' => 1,
        'BinaryAnvil_JobManager' => 1,
        'Magento_AdminNotification' => 1,
        'Magento_Ui' => 1,
        'BinaryAnvil_FederationFacebook' => 1,
        'BinaryAnvil_ExtendFederationFacebook' => 1,
        'Magento_Indexer' => 1,
        'BinaryAnvil_ExtendJobManager' => 1,
        'Magento_Search' => 1,
        'Magento_Cms' => 1,
        'Magento_Quote' => 1,
        'Magento_Catalog' => 1,
        'BinaryAnvil_Seo' => 1,
        'Magento_CatalogInventory' => 1,
        'Magento_Config' => 1,
        'BinaryAnvil_Toolbar' => 1,
        'Magento_Rule' => 1,
        'Magento_CacheInvalidate' => 1,
        'Magento_Payment' => 1,
        'Magento_SalesSequence' => 1,
        'Magento_Sales' => 1,
        'Lexim_AbandonedCart' => 1,
        'Lexim_InventoryIntegration' => 1,
        'Lexim_Localretailer' => 1,
        'Lexim_Override' => 1,
        'Lexim_Product' => 1,
        'Lexim_SizeChart' => 1,
        'Magento_SalesRule' => 1,
        'Magento_Authorization' => 1,
        'Magento_Security' => 1,
        'Magento_AdvancedCatalog' => 1,
        'Magento_AdvancedCheckout' => 1,
        'Magento_AdvancedPricingImportExport' => 1,
        'Magento_AdvancedRule' => 1,
        'Magento_AdvancedSalesRule' => 1,
        'Magento_AdvancedSearch' => 1,
        'Magento_Amqp' => 0,
        'Magento_User' => 1,
        'Magento_AsynchronousOperations' => 1,
        'Magento_AdminGws' => 1,
        'Magento_Checkout' => 1,
        'ParadoxLabs_TokenBase' => 1,
        'Magento_Backup' => 1,
        'Magento_CatalogRule' => 1,
        'Magento_CustomerCustomAttributes' => 1,
        'Magento_Vault' => 1,
        'Magento_Bundle' => 1,
        'Magento_BundleImportExport' => 1,
        'Magento_BundleImportExportStaging' => 1,
        'Magento_CatalogUrlRewrite' => 1,
        'Magento_PageCache' => 1,
        'Magento_Captcha' => 1,
        'Amazon_Core' => 0,
        'Magento_Integration' => 1,
        'Magento_MediaStorage' => 1,
        'Magento_CatalogImportExport' => 1,
        'Magento_CatalogImportExportStaging' => 1,
        'Amazon_Login' => 0,
        'Magento_Staging' => 1,
        'Magento_CatalogSearch' => 1,
        'Magento_Cron' => 1,
        'Magento_Msrp' => 1,
        'Magento_Widget' => 1,
        'Magento_LayeredNavigation' => 1,
        'Magento_Downloadable' => 1,
        'Magento_GiftCard' => 1,
        'Magento_UrlRewrite' => 1,
        'Magento_CatalogWidget' => 1,
        'Amazon_Payment' => 0,
        'Magento_CheckoutAgreements' => 1,
        'Magento_CheckoutStaging' => 1,
        'Magento_ConfigurableProduct' => 1,
        'Magento_CmsStaging' => 1,
        'Magento_CmsUrlRewrite' => 1,
        'Magento_Swatches' => 1,
        'Magento_ConfigurableImportExport' => 1,
        'Magento_CatalogRuleConfigurable' => 1,
        'Magento_ConfigurableProductSales' => 1,
        'Magento_Wishlist' => 1,
        'Magento_Contact' => 1,
        'Magento_Cookie' => 1,
        'Magento_Email' => 1,
        'Magento_CurrencySymbol' => 1,
        'Magento_CustomAttributeManagement' => 1,
        'Magento_Paypal' => 1,
        'Magento_Analytics' => 1,
        'Magento_CustomerBalance' => 1,
        'Magento_CustomerSegment' => 1,
        'Magento_CustomerFinance' => 1,
        'Magento_CustomerImportExport' => 1,
        'Magento_VersionsCms' => 1,
        'Magento_Cybersource' => 0,
        'Magento_Deploy' => 1,
        'Magento_Developer' => 1,
        'Magento_Dhl' => 0,
        'Magecomp_Matrixrate' => 1,
        'Magento_ProductAlert' => 1,
        'Magento_ImportExport' => 1,
        'Magento_Reports' => 1,
        'BinaryAnvil_Customer' => 1,
        'Magento_Elasticsearch' => 1,
        'Magento_SendFriend' => 1,
        'Magento_EncryptionKey' => 1,
        'Magento_Enterprise' => 1,
        'Magento_Eway' => 0,
        'Magento_Fedex' => 1,
        'Magento_TargetRule' => 1,
        'Magento_GiftCardAccount' => 1,
        'Magento_GiftCardImportExport' => 1,
        'Magento_Tax' => 1,
        'Magento_GiftMessage' => 1,
        'Magento_GiftMessageStaging' => 1,
        'Magento_Weee' => 1,
        'Magento_GiftWrapping' => 1,
        'Magento_GiftWrappingStaging' => 1,
        'Magento_GoogleAdwords' => 1,
        'Magento_GoogleAnalytics' => 1,
        'Magento_GoogleOptimizer' => 1,
        'Magento_GoogleOptimizerStaging' => 1,
        'Magento_Banner' => 1,
        'Magento_GroupedImportExport' => 1,
        'Magento_GroupedProduct' => 1,
        'Magento_GroupedProductStaging' => 1,
        'Magento_DownloadableImportExport' => 1,
        'BinaryAnvil_InfinityTheme' => 1,
        'Magento_InstantPurchase' => 1,
        'Magento_Multishipping' => 1,
        'Magento_WebsiteRestriction' => 1,
        'BinaryAnvil_LayeredNavigation' => 1,
        'Magento_LayeredNavigationStaging' => 1,
        'Magento_Logging' => 1,
        'Magento_Marketplace' => 0,
        'Magento_CatalogEvent' => 1,
        'Magento_MessageQueue' => 1,
        'BinaryAnvil_SizeChart' => 1,
        'Magento_MsrpStaging' => 1,
        'Magento_MultipleWishlist' => 1,
        'Magento_Shipping' => 1,
        'Magento_MysqlMq' => 1,
        'Magento_NewRelicReporting' => 1,
        'Magento_Newsletter' => 1,
        'Magento_OfflinePayments' => 1,
        'Magento_OfflineShipping' => 1,
        'Fastly_Cdn' => 1,
        'Klarna_Core' => 0,
        'Magento_PaymentStaging' => 1,
        'Klarna_Ordermanagement' => 0,
        'Magento_PaypalOnBoarding' => 0,
        'Magento_Persistent' => 1,
        'Magento_PersistentHistory' => 1,
        'Magento_PricePermissions' => 1,
        'Magento_GiftRegistry' => 1,
        'Magento_ProductVideo' => 1,
        'Magento_CatalogStaging' => 1,
        'Magento_PromotionPermissions' => 1,
        'BinaryAnvil_OrderLogix' => 1,
        'Magento_QuoteAnalytics' => 1,
        'Magento_ReleaseNotification' => 1,
        'Magento_Reminder' => 1,
        'Magento_ConfigurableProductStaging' => 1,
        'Magento_RequireJs' => 1,
        'Magento_ResourceConnections' => 1,
        'Magento_Review' => 1,
        'Magento_ReviewAnalytics' => 1,
        'Magento_ReviewStaging' => 1,
        'Magento_Reward' => 1,
        'Magento_RewardStaging' => 1,
        'Magento_Rma' => 1,
        'Magento_RmaStaging' => 1,
        'Magento_Robots' => 1,
        'Magento_Rss' => 1,
        'Magento_CatalogRuleStaging' => 1,
        'Magento_Braintree' => 0,
        'Magento_SalesAnalytics' => 1,
        'Magento_Signifyd' => 1,
        'Magento_SalesInventory' => 1,
        'BinaryAnvil_ExtendMatrixrate' => 1,
        'Magento_SalesRuleStaging' => 1,
        'Magento_Authorizenet' => 1,
        'Magento_SampleData' => 0,
        'Magento_ScalableCheckout' => 1,
        'Magento_ScalableInventory' => 1,
        'Magento_ScalableOms' => 1,
        'Magento_ScheduledImportExport' => 1,
        'Magento_CatalogPermissions' => 1,
        'Magento_SearchStaging' => 1,
        'Magento_CatalogAnalytics' => 1,
        'Dotdigitalgroup_Email' => 0,
        'Temando_Shipping' => 0,
        'Magento_SalesArchive' => 1,
        'Magento_Sitemap' => 1,
        'Magento_Solr' => 0,
        'Magento_CatalogInventoryStaging' => 1,
        'BinaryAnvil_Ratings' => 1,
        'Magento_Support' => 1,
        'Magento_Swagger' => 0,
        'BinaryAnvil_Swatches' => 1,
        'Magento_SwatchesLayeredNavigation' => 1,
        'Magento_GiftCardStaging' => 1,
        'Magento_BundleStaging' => 1,
        'Magento_TaxImportExport' => 1,
        'BinaryAnvil_Newsletter' => 1,
        'Magento_Translation' => 1,
        'BinaryAnvil_ExtendUi' => 1,
        'Magento_Ups' => 1,
        'Magento_CatalogUrlRewriteStaging' => 1,
        'Magento_CustomerAnalytics' => 1,
        'Magento_Usps' => 1,
        'Magento_Variable' => 1,
        'Klarna_Kp' => 0,
        'Magento_Version' => 1,
        'Magento_BannerCustomerSegment' => 1,
        'Magento_VisualMerchandiser' => 1,
        'Magento_Webapi' => 1,
        'Magento_WebapiSecurity' => 1,
        'Magento_Invitation' => 1,
        'Magento_DownloadableStaging' => 1,
        'Magento_WeeeStaging' => 1,
        'Magento_GoogleTagManager' => 1,
        'Magento_ProductVideoStaging' => 1,
        'Magento_WishlistAnalytics' => 1,
        'Magento_Worldpay' => 0,
        'Mageplaza_Core' => 1,
        'Mageplaza_GoogleRecaptcha' => 1,
        'Mageplaza_Reports' => 1,
        'Mageplaza_Smtp' => 1,
        'Mageplaza_SocialLogin' => 1,
        'ParadoxLabs_Authnetcim' => 1,
        'BinaryAnvil_ExtendAuthnetcim' => 1,
        'Shopial_Facebook' => 0,
        'Aheadworks_OneStepCheckout' => 1,
        'Vertex_Tax' => 1
    ],
    'admin_user' => [
        'locale' => [
            'code' => [
                'en_US'
            ]
        ]
    ]
];
